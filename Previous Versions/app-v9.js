...
// ==========================================
    ...
    function openEditModal(comm) {
        ...
        setTimeout(function() {
            var firstInput = document.getElementById('edit-student-name');
            if (firstInput) {
            firstInput.focus();
            }
        }, 100);
        // Disable background from tab order
    var appSection = document.getElementById('app-section');
    if (appSection) {
        appSection.setAttribute('inert', '');
   ...
   function closeEditModal() {
    ...
    var appSection = document.getElementById('app-section');
    if (appSection) {
        appSection.removeAttribute('inert');
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' || e.keyCode === 27) {
            ...
            // Close any open three-dot menus
            document.querySelectorAll('.comm-actions.show').forEach(function(div) {
                div.classList.remove('show');
            });
            document.querySelectorAll('.menu-btn.active').forEach(function(btn) {
                btn.classList.remove('active');
            });
            
            // Close filters if expanded (optional convenience)
   ...
   // Trap focus inside modal when open (Safari-aggressive version)
    var modalFocusTrap = function(e) {
        if (!editModal || editModal.style.display !== 'flex') return;
        
        if (e.key === 'Tab' || e.keyCode === 9) {
            // Get all focusable elements every time (in case DOM changed)
            var focusableElements = editModal.querySelectorAll(
            ...
            // Prevent default ALWAYS in modal
            e.preventDefault();
            e.stopPropagation();
            ...
            // Force focus with slight delay for Safari
            setTimeout(function() {
    ...
    document.addEventListener('keydown', modalFocusTrap, true); // Note: added 'true' for capture phase
